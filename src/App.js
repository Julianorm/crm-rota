...
const importClients=async(rows)=>{setLoading(true);await supabase.from('clients').delete().eq('user_id',user.id);const toInsert=rows.map(cols=>({user_id:user.id,name:String(cols[0]).trim(),route:String(cols[1]).trim(),inactive:cols[2]&&String(cols[2]).trim().toLowerCase()==='inativo'}));const{error}=await supabase.from('clients').insert(toInsert);if(error){showToast('Erro ao salvar clientes.','error');setLoading(false);return}await loadClients();setSales([]);setSelectedRoute('');setDailyGoal('');setLoading(false);showToast(`${toInsert.length} clientes importados!`)}
...
<div style={{fontSize:11,color:MUTED,marginBottom:8,lineHeight:1.5}}>Copie as células do Excel (cabeçalho: Cliente, Rota, Status) e cole abaixo:</div>
